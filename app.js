...
window.editarCampo = function(id) {
  onValue(ref(db, `campos/${id}`), snapshot => {
    ...
    // Reescribir el guardar para actualizar en vez de crear
    campoForm.onsubmit = function(e) {
      ...
      update(ref(db, `campos/${id}`), nuevo).then(() => {
        ...
        parcelas = [];
        renderParcelas();
        campoForm.onsubmit = null; // volver al estado normal
        window.location.reload(); // recargar para evitar múltiples botones editando
      });
    };
  }, { onlyOnce: true });
};
